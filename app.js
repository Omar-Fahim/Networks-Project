...
var places = [];
places.push("annapurna");
places.push("bali");
places.push("inca");
places.push("paris");
places.push("rome");
places.push("santorini");
   ...
   // if (err)  throw err;
    ...
    //var obj =  db.collection('myCollection').find(query).toArray((err, req) => {
     ...
     // {
        ...
        {
         // db.collection('myCollection').insertOne({username: username, password: password, list:[]});
          // After succesful registration, user then redirected to the login page
...
app.post('/search',function(req,res){
  ...
  {
  if(places[0].includes(substring)){data1="annapurna"}
  if(places[1].includes(substring)){data2="bali"}
  if(places[2].includes(substring)){data3="inca"}
  if(places[3].includes(substring)){data4="paris"}
  if(places[4].includes(substring)){data5="rome"}
  if(places[5].includes(substring)){data6="santorini"}
  ...
  //{
   ...
   // db.collection('myCollection').find(query).toArray().then(function(array){
    //  var obj = array[0].list;
     ...
     // else
     // {
        //db.collection('myCollection').updateOne(query, {$push: {list:'santorini'}});
...
app.post('/rome', function(req, res){
  ...
  //     else
  //     {
  //       db.collection('myCollection').updateOne(query, {$push: {list:'rome'}});
...
app.post('/paris', function(req, res){
  ...
  //     else
  //     {
  //       db.collection('myCollection').updateOne(query, {$push: {list:'paris'}});
...
app.post('/inca', function(req,res){
  ...
  //     else
  //     {
  //       db.collection('myCollection').updateOne(query, {$push: {list:'inca'}});
...
app.post('/bali', function(req, res){
  ...
  //     else
  //     {
  //       db.collection('myCollection').updateOne(query, {$push: {list:'bali'}});
...
app.post('/annapurna', function(req, res){
  ...
  //   else
  //   {
  //     db.collection('myCollection').updateOne(query, {$push: {list:'annapurna'}});
